define('ECIMS_Index', () =>
{
	...
	class ECIMS_Index extends Component
	{
		...
		componentDidMount()
		{
			...
			//window.Cooperative_EconomicSurvey	=	new ECIMS_Cooperative_EconomicSurvey()
			...
			window.System_Session = () =>
			{
				if (cookies.get('User_Identity') != undefined)
				{
					...
					if (!(this.state['Header_Visible']) && !(this.state['Sidebar_Visible']))
					{
						...
						{
							let get_Callback_Data = set_Callback_Data
							...
							if (get_Callback_Data.length != 0)
							{
								System_Index.setState({ 'Header_Visible' : true, 'Sidebar_Visible' : true }, () =>
								{
									...
									if (location.pathname != '/Cooperative/Associated')
										System_Navigator.Forward('/Cooperative/Associated')
						...
						<main className = 'main'>
							...
							<BrowserRouter ref = { this.Element['System_BrowserRouter'] }>
								...
								<Switch>
									...
									<Route path = '/Cooperative/Add' component = { () => <ECIMS_Cooperative_EconomicSurvey/> }/>
									<Route path = '/Cooperative/Create' component = { () => <ECIMS_Cooperative_EconomicSurvey/> }/>
									<Route path = '/Cooperative/List' component = { () => <ECIMS_Cooperative_List/> }/>
									<Route path = '/Cooperative/Associated' component = { () => <ECIMS_Cooperative_List/> }/>
									<Route path = '/Cooperative/Join' component = { () => <ECIMS_Cooperative_List/> }/>
