define('ECIMS_Cooperative_EconomicSurvey', () =>
{
	class ECIMS_Cooperative_EconomicSurvey extends ECIMS_Cooperative
	{
		constructor()
		{
			if (System_Mode == 'Development')
				console.log('Succesfully Loaded : ECIMS_Cooperative_EconomicSurvey Class')

			super()

			this.set_Component_Mutablity('Cooperative_EconomicSurvey', true)

			this.get_Table_OrganizationalStructure_Member_Element = []

			this.get_Table_OrganizationalStructure_Member_Element.push
			(
				set_Data =>
				{
					return	<div>
			...
			(
				set_Data =>
				{
					return	<div>
								...
								<button className = 'btn btn-theme waves-effect DataTable_Button' type = 'button'>
									Delete
								</button>
							</div>
				}
			...
			this.get_Table_OrganizationalStructure_Committee_Element = []

			this.get_Table_OrganizationalStructure_Committee_Element.push
			(
				set_Data =>
				{
					return	<div>
		...
		Select_Cooperative_EconomicSurvey(set_Action, set_Data)
		{
			let get_FormData = new FormData()

			if (set_Action == 'Update Economic Survey' || set_Action == 'View Economic Survey')
			{
				if (set_Action == 'Update Economic Survey')
					this.set_Component_Mutablity('Cooperative_EconomicSurvey', true)
				...
				ECIMS_Main_HTTP.Select(get_FormData, set_Callback_Data =>
				{
					...
					$(this.Element['Cooperative_Identity'].current).val(set_Callback_Data[0]['Cooperative_EconomicSurvey.Cooperative_Identity'])

					this.set_Cooperative_EconomicSurvey_Data(JSON.parse(set_Callback_Data[0]['EconomicSurvey_Description']))
					$('a[href = "#Tab_Cooperative_Add"]').trigger('click')
				})
			}
		}

		Manage_Cooperative_EconomicSurvey(set_Action, set_Data, set_Element)
		{
			...
			{
				System_Dialog.Alert_Text = 'This Cooperative will be inserted.'

				get_FormData = set_Data
				get_FormData.append('Action', 'Insert_Cooperative_EconomicSurvey')
				get_FormData.append('User_Identity', Session_User_Identity())
				get_FormData.append('Cooperative_State', 'Pending')
			}

			else if (set_Action == 'Update : Specific')
			{
				System_Dialog.Alert_Text = 'This Cooperative will be updated.'

				get_FormData = set_Data
				get_FormData.append('Action', 'Update_Cooperative_EconomicSurvey')
				get_FormData.append('Update_Action', 'Update : Specific')
				get_FormData.append('User_Identity', Session_User_Identity())
			}

			else if (set_Action == 'Cancel Request' || set_Action == 'Re-request')
			{
				...
				if (set_Action == 'Cancel Request')
				{
					System_Dialog.Alert_Text = 'This Cooperative Registration will be cancelled.'
					get_FormData.append('Cooperative_State', 'Cancelled')
				}

				else if (set_Action == 'Re-request')
				{
					System_Dialog.Alert_Text = 'This Cooperative Registration will be Re-requested.'
					get_FormData.append('Cooperative_State', 'Pending')
				}
			}
			...
			get_Promise.then((set_Resolve_Data) =>
			{
				if (set_Resolve_Data == 'Yes')
				{
					...
					{
						if (set_Action == 'Insert : Specific' || set_Action == 'Update : Specific')
						{
							if (set_Action == 'Insert : Specific')
							{
								let get_FormData = new FormData()
								...
								get_FormData.append('Action', 'Select_Cooperative')
								get_FormData.append('Select_Action', 'Select : Specific : { Cooperative_Inserted_By }')
								get_FormData.append('Cooperative_Inserted_By', Session_User_Identity())

								ECIMS_Main_HTTP.Select(get_FormData, set_Callback_Data =>
								{
									this.state['Cooperative_Requested'] = set_Callback_Data
									this.Initialize_Table_Cooperative_Requested()
								})
							}

							else if (set_Action == 'Update : Specific')
							{
								this.Table_Cooperative_Requested.DataTable().rows().every((set_RowIndex, set_TableLoop, set_RowLoop) =>
								{
									let get_Data = this.Table_Cooperative_Requested.DataTable().row(set_RowIndex).data()

									if (get_Data['Cooperative_Identity'] == $(this.Element['Cooperative_Identity'].current).val())
									{
										get_Data['Cooperative_Name'] = $(this.Element['Cooperative_Name'].current).val()

										this.Table_Cooperative_Requested.DataTable().row(set_RowIndex).remove().draw()
										this.Table_Cooperative_Requested.DataTable().row.add(get_Data).draw()
							...
							$('form').trigger('reset').removeClass('was-validated')
							$('#Carousel_Cooperative_EconomicSurvey').carousel(0)
						}

						else if (set_Action == 'Cancel Request' || set_Action == 'Re-request')
						{
							let get_Element = set_Element != null ? $(set_Element).parent() : false
							let get_RowIndex = this.Table_Cooperative_Requested.DataTable().row(get_Element).index()
							let get_Data = this.Table_Cooperative_Requested.DataTable().row(get_RowIndex).data()

							if (set_Action == 'Cancel Request')
								get_Data['Cooperative_State'] = 'Cancelled'

							else if (set_Action == 'Re-request')
								get_Data['Cooperative_State'] = 'Pending'

							this.Table_Cooperative_Requested.DataTable().row(get_RowIndex).remove().draw()
							this.Table_Cooperative_Requested.DataTable().row.add(get_Data).draw()
						}					
		...
		get_Cooperative_EconomicSurvey_Data()
		{
			...
			for (let set_Key in get_Form_Cooperative_1_Data)
			{
				if (Array.isArray(get_Form_Cooperative_1_Data[set_Key]))
					get_FormData.append(set_Key, JSON.stringify(get_Form_Cooperative_1_Data[set_Key]))

				else
					get_FormData.append(set_Key, get_Form_Cooperative_1_Data[set_Key])
			}

			for (let set_Key in get_Form_Cooperative_2_Data)
			{
				if (Array.isArray(get_Form_Cooperative_2_Data[set_Key]))
					get_FormData.append(set_Key, JSON.stringify(get_Form_Cooperative_2_Data[set_Key]))

				else
					get_FormData.append(set_Key, get_Form_Cooperative_2_Data[set_Key])
			}

			for (let set_Key in get_Form_Cooperative_ShareCapital_Data)
			{
				if (Array.isArray(get_Form_Cooperative_ShareCapital_Data[set_Key]))
					get_FormData.append(set_Key, JSON.stringify(get_Form_Cooperative_ShareCapital_Data[set_Key]))

				else
					get_FormData.append(set_Key, get_Form_Cooperative_ShareCapital_Data[set_Key])
			}

			for (let set_Key in get_Form_Cooperative_EconomicAspect_Data)
			{
				if (Array.isArray(get_Form_Cooperative_EconomicAspect_Data[set_Key]))
					get_FormData.append(set_Key, JSON.stringify(get_Form_Cooperative_EconomicAspect_Data[set_Key]))

				else
					get_FormData.append(set_Key, get_Form_Cooperative_EconomicAspect_Data[set_Key])
			}

			for (let set_Key in get_Form_Cooperative_FinancialAspect_Data)
			{
				if (Array.isArray(get_Form_Cooperative_FinancialAspect_Data[set_Key]))
					get_FormData.append(set_Key, JSON.stringify(get_Form_Cooperative_FinancialAspect_Data[set_Key]))

				else
					get_FormData.append(set_Key, get_Form_Cooperative_FinancialAspect_Data[set_Key])
			}

			for (let set_Key in get_Form_Cooperative_TechnicalAspect_Data)
			{
				if (Array.isArray(get_Form_Cooperative_TechnicalAspect_Data[set_Key]))
					get_FormData.append(set_Key, JSON.stringify(get_Form_Cooperative_TechnicalAspect_Data[set_Key]))

				else
					get_FormData.append(set_Key, get_Form_Cooperative_TechnicalAspect_Data[set_Key])
			}

			return get_FormData
		}

		set_Cooperative_EconomicSurvey_Data(set_Data)
		{
			$('form').on('submit', set_Event => { set_Event.preventDefault() })

			this.state['Cooperative_Member_Composition'] = []
			this.state['Cooperative_Member_Projected_Growth'] = []
			this.state['Cooperative_Member_Projected_Growth_Key_Counter'] = 0

			this.state['EconomicAspect_Strategy'] = []
			this.state['EconomicAspect_Strategy_Key_Counter'] = 0
			this.state['EconomicAspect_Activity'] = []
			this.state['EconomicAspect_Activity_Key_Counter'] = 0

			this.state['FinancialAspect_Capitalization_Generate_Capital'] = []
			this.state['FinancialAspect_Capitalization_Internal_Capital_Strategy'] = []
			this.state['FinancialAspect_Capitalization_Internal_Capital_Strategy_Key_Counter'] = 0
			this.state['FinancialAspect_Revenue_Projected'] = []
			this.state['FinancialAspect_Revenue_Projected_Key_Counter'] = 0
			this.state['FinancialAspect_Expense_Estimated'] = []
			this.state['FinancialAspect_Expense_Estimated_Key_Counter'] = 0
			this.state['FinancialAspect_Organization_Investment'] = []

			this.state['TechnicalAspect_Equipment'] = []
			this.state['TechnicalAspect_Machinery'] = []
			this.state['TechnicalAspect_Facility'] = []
			this.state['TechnicalAspect_Procurement_Mode'] = []
			this.state['TechnicalAspect_Operator_Background'] = []
			this.state['TechnicalAspect_Operator_Background_Key_Counter'] = 0

			this.state['OrganizationalStructure_Member_Eligibility'] = []
			this.state['OrganizationalStructure_Member_Eligibility_Key_Counter'] = 0
			this.state['OrganizationalStructure_Program'] = []
			this.state['OrganizationalStructure_Program_Key_Counter'] = 0

			this.state['OrganizationalStructure_Member'] = []
			this.state['OrganizationalStructure_Member_Chart'] = []
			this.state['OrganizationalStructure_Committee'] = []
			this.state['OrganizationalStructure_Committee_Member'] = []

			const Potekk = (() =>
			{
				this.Update_Interface()

				System_Manipulator.set_Textarea_Autosize({ 'Element' : $('textarea') })

				if (this.check_Component_Mutablity('Cooperative_EconomicSurvey'))
					return
				...
				/*Object.freeze(this.state['Cooperative_Member_Projected_Growth'])
				Object.freeze(this.state['EconomicAspect_Strategy'])
				Object.freeze(this.state['EconomicAspect_Activity'])

				Object.freeze(this.state['FinancialAspect_Capitalization_Internal_Capital_Strategy'])
				Object.freeze(this.state['FinancialAspect_Revenue_Projected'])
				Object.freeze(this.state['FinancialAspect_Expense_Estimated'])

				Object.freeze(this.state['TechnicalAspect_Operator_Background'])

				Object.freeze(this.state['OrganizationalStructure_Member_Eligibility'])
				Object.freeze(this.state['OrganizationalStructure_Program'])*/
			})
			...
			const Initialize_Cooperative_1 = (() =>
			{
				...
				$(this.Element['Cooperative_Name'].current).val(set_Data['Cooperative_1']['Cooperative_Name'])
				$(this.Element['Cooperative_Background'].current).val(set_Data['Cooperative_1']['Cooperative_Background'])
				$(this.Element['Cooperative_Rationale'].current).val(set_Data['Cooperative_1']['Cooperative_Rationale'])
			...
			const Initialize_Cooperative_2 = (() =>
			{
				...
				$(this.Element['Cooperative_Type'].current).val(set_Data['Cooperative_2']['Cooperative_Type']).change()

				System_Manipulator.set_Select({ 'Element' : $(this.Element['Cooperative_Category'].current)})
				$(this.Element['Cooperative_Category'].current).val(set_Data['Cooperative_2']['Cooperative_Category']).change()

				System_Manipulator.set_Select({ 'Element' : $(this.Element['Cooperative_Region'].current)})
				$(this.Element['Cooperative_Region'].current).val(set_Data['Cooperative_2']['Cooperative_Region']).change()

				System_Manipulator.set_Select({ 'Element' : $(this.Element['Cooperative_Province'].current)})
				$(this.Element['Cooperative_Province'].current).val(set_Data['Cooperative_2']['Cooperative_Province']).change()

				System_Manipulator.set_Select({ 'Element' : $(this.Element['Cooperative_Locality'].current)})
				$(this.Element['Cooperative_Locality'].current).val(set_Data['Cooperative_2']['Cooperative_Locality']).change()

				System_Manipulator.set_Select({ 'Element' : $(this.Element['Cooperative_Operation_Area'].current)})
				$(this.Element['Cooperative_Operation_Area'].current).val(set_Data['Cooperative_2']['Cooperative_Operation_Area']).change()

				System_Manipulator.set_Select({ 'Element' : $(this.Element['Cooperative_Common_Bond'].current)})
				$(this.Element['Cooperative_Common_Bond'].current).val(set_Data['Cooperative_2']['Cooperative_Common_Bond']).change()

				System_Manipulator.set_Select({ 'Element' : $(this.Element['Cooperative_Organization_Registration'].current)})
				$(this.Element['Cooperative_Organization_Registration'].current).val(set_Data['Cooperative_2']['Cooperative_Organization_Registration']).change()

				set_Data['Cooperative_2']['Cooperative_Member_Composition'].map(set_Data =>
				{
					this.state['Cooperative_Member_Composition'].push(set_Data)
				})
				...
				set_Data['Cooperative_2']['Cooperative_Member_Projected_Growth'].map(set_Data =>
				{
					this.state['Cooperative_Member_Projected_Growth'].push(set_Data)
					this.state['Cooperative_Member_Projected_Growth_Key_Counter'] ++
				})
			})()
			...
			const Initialize_Cooperative_ShareCapital = (() =>
			{
				...
				$(this.Element['ShareCapital_Authorized'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Authorized'])
				$(this.Element['ShareCapital_Issued'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Issued'])
				$(this.Element['ShareCapital_Unissued'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Unissued'])
				$(this.Element['ShareCapital_Subscribed'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Subscribed'])
				$(this.Element['ShareCapital_Unsubscribed'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Unsubscribed'])
				$(this.Element['ShareCapital_Called-Up'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Called-Up'])
				$(this.Element['ShareCapital_Uncalled'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Uncalled'])
				$(this.Element['ShareCapital_Paid-Up'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Paid-Up'])
				$(this.Element['ShareCapital_Unpaid'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Unpaid'])
				$(this.Element['ShareCapital_Reserve'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Reserve'])
				$(this.Element['ShareCapital_Par_Value'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Par_Value'])
				$(this.Element['ShareCapital_Share_Sold'].current).val(set_Data['Cooperative_ShareCapital']['ShareCapital_Share_Sold'])
			})()
			...
			const Initialize_Cooperative_EconomicAspect = (() =>
			{
				...
				$(this.Element['EconomicAspect_Identical_Cooperative_Plan'].current).val(set_Data['Cooperative_EconomicAspect']['EconomicAspect_Identical_Cooperative_Plan']).change()

				System_Manipulator.set_Select({ 'Element' : $(this.Element['EconomicAspect_Target_Market'].current)})
				$(this.Element['EconomicAspect_Target_Market'].current).val(set_Data['Cooperative_EconomicAspect']['EconomicAspect_Target_Market']).change()

				set_Data['Cooperative_EconomicAspect']['EconomicAspect_Strategy'].map(set_Data =>
				{
					this.state['EconomicAspect_Strategy'].push(set_Data)
					this.state['EconomicAspect_Strategy_Key_Counter'] ++
				})

				set_Data['Cooperative_EconomicAspect']['EconomicAspect_Activity'].map(set_Data =>
				{
					this.state['EconomicAspect_Activity'].push(set_Data)
					this.state['EconomicAspect_Activity_Key_Counter'] ++
				})
			})()
			...
			const Initialize_Cooperative_FinancialAspect = (() =>
			{
				...
				$(this.Element['FinancialAspect_Capitalization_Generate_Capital'].current).val(set_Data['Cooperative_FinancialAspect']['FinancialAspect_Capitalization_Generate_Capital'])

				set_Data['Cooperative_FinancialAspect']['FinancialAspect_Capitalization_Internal_Capital_Strategy'].map(set_Data =>
				{
					this.state['FinancialAspect_Capitalization_Internal_Capital_Strategy'].push(set_Data)
					this.state['FinancialAspect_Capitalization_Internal_Capital_Strategy_Key_Counter'] ++
				})

				set_Data['Cooperative_FinancialAspect']['FinancialAspect_Revenue_Projected'].map(set_Data =>
				{
					this.state['FinancialAspect_Revenue_Projected'].push(set_Data)
					this.state['FinancialAspect_Revenue_Projected_Key_Counter'] ++
				})

				set_Data['Cooperative_FinancialAspect']['FinancialAspect_Expense_Estimated'].map(set_Data =>
				{
					this.state['FinancialAspect_Expense_Estimated'].push(set_Data)
					this.state['FinancialAspect_Expense_Estimated_Key_Counter'] ++
				})

				set_Data['Cooperative_FinancialAspect']['FinancialAspect_Organization_Investment'].map(set_Data =>
				{
					this.state['FinancialAspect_Organization_Investment'].push(set_Data)
				})
			...
			const Initialize_Cooperative_TechnicalAspect = (() =>
			{
				...
				set_Data['Cooperative_TechnicalAspect']['TechnicalAspect_Equipment'].map(set_Data =>
				{
					this.state['TechnicalAspect_Equipment'].push(set_Data)
				})
				...
				set_Data['Cooperative_TechnicalAspect']['TechnicalAspect_Machinery'].map(set_Data =>
				{
					this.state['TechnicalAspect_Machinery'].push(set_Data)
				})
				...
				set_Data['Cooperative_TechnicalAspect']['TechnicalAspect_Facility'].map(set_Data =>
				{
					this.state['TechnicalAspect_Facility'].push(set_Data)
				})
				...
				set_Data['Cooperative_TechnicalAspect']['TechnicalAspect_Procurement_Mode'].map(set_Data =>
				{
					this.state['TechnicalAspect_Procurement_Mode'].push(set_Data)
				})
				...
				set_Data['Cooperative_TechnicalAspect']['TechnicalAspect_Operator_Background'].map(set_Data =>
				{
					this.state['TechnicalAspect_Operator_Background'].push(set_Data)
					this.state['TechnicalAspect_Operator_Background_Key_Counter'] ++
				})
			...
			const Initialize_Cooperative_OrganizationalStructure = (() =>
			{
				...
				set_Data['Cooperative_OrganizationalStructure']['OrganizationalStructure_Member_Eligibility'].map(set_Data =>
				{
					this.state['OrganizationalStructure_Member_Eligibility'].push(set_Data)
					this.state['OrganizationalStructure_Member_Eligibility_Key_Counter'] ++
				})

				set_Data['Cooperative_OrganizationalStructure']['OrganizationalStructure_Program'].map(set_Data =>
				{
					this.state['OrganizationalStructure_Program'].push(set_Data)
					this.state['OrganizationalStructure_Program_Key_Counter'] ++
				})

				this.Table_OrganizationalStructure_Member = $(this.Element['Table_OrganizationalStructure_Member'].current)				
				this.Table_OrganizationalStructure_Member.DataTable().clear().destroy()
				...
				({
					...
					'initComplete'	:	() =>
					{
						setTimeout(() =>
						{
							set_Data['Cooperative_OrganizationalStructure']['OrganizationalStructure_Member'].map(set_Data =>
							{
								this.Table_OrganizationalStructure_Member.DataTable().row.add
								([
									set_Data['Position_Name'],
									set_Data['Member_Fullname'],
									set_Data['Member_Appointment_Status'],
									set_Data['Member_Eligibility'],
									set_Data['Member_Compensation'],
									''
								])
							})
					...
					'createdRow'	:	(set_Row, set_Data, set_Index) =>
					{
						...
						{
							let Component = this.get_Table_OrganizationalStructure_Member_Element[set_Index]
						...
						$(set_Row).children('td').each((set_Index, set_Element) =>
						{
							...
							{
								get_Element.attr('readOnly', true)
								get_Element.val(set_Data[set_Index])
							...
							else if (get_Element.prop('tagName') == 'SELECT')
							{
								...
								get_Element.prop('disabled', true)
								get_Element.val(set_Data[set_Index]).change()
								get_Element.on('change', get_Event)
							}
				...
				this.Table_OrganizationalStructure_Committee.DataTable().clear().destroy()
				...
				({
					...
					'initComplete'	:	() =>
					{
						setTimeout(() =>
						{
							set_Data['Cooperative_OrganizationalStructure']['OrganizationalStructure_Committee'].map(set_Data =>
							{
								this.Table_OrganizationalStructure_Committee.DataTable().row.add
								([
									set_Data['Committee_Name'],
									set_Data['Committee_Chairman'],
									set_Data['Committee_Member'],
									set_Data['Committee_Description'],
									''
								])
							})
						}, 0)
					...
					'createdRow'	:	(set_Row, set_Data, set_Index) =>
					{
						...
						$(set_Row).children('td').each((set_Index, set_Element) =>
						{
							let Component = this.get_Table_OrganizationalStructure_Committee_Element[set_Index]
						...
						$(set_Row).children('td').each((set_Index, set_Element) =>
						{
							...
							else if (get_Element.prop('tagName') == 'SELECT')
							{
								...
								({
									'Element'	:	get_Element,
									'Array'		:	set_Data[set_Index]
								})

								$(get_Element).next().find('input').attr('readOnly', true)
			...
			const Initialize_Cooperative_OrganizationalStructure_Remark = (() =>
			{
				...
				$('input[name = "OrganizationalStructure_Remark_Day"]').val(set_Data['Cooperative_OrganizationalStructure_Remark']['OrganizationalStructure_Remark_Day'])
				$('input[name = "OrganizationalStructure_Remark_Month"]').val(set_Data['Cooperative_OrganizationalStructure_Remark']['OrganizationalStructure_Remark_Month'])
				$('input[name = "OrganizationalStructure_Remark_Year"]').val(set_Data['Cooperative_OrganizationalStructure_Remark']['OrganizationalStructure_Remark_Year'])
				$('input[name = "OrganizationalStructure_Remark_Address"]').val(set_Data['Cooperative_OrganizationalStructure_Remark']['OrganizationalStructure_Remark_Address'])

				let get_Cooperative_OrganizationalStructure_Remark_Member = set_Data['Cooperative_OrganizationalStructure_Remark']['Cooperative_OrganizationalStructure_Remark_Member']

				$(this.Element['Table_Cooperative_OrganizationalStructure_Remark'].current).children('tbody').children('tr').each((set_Index_1, set_Element) =>
				{
					$($(set_Element).children('td')).each((set_Index_2, set_Element) =>
					{
						let get_Element = $(set_Element).children('input')

						if (get_Element.attr('name') == 'OrganizationalStructure_Remark_Member')
						{
							get_Element.val(get_Cooperative_OrganizationalStructure_Remark_Member[set_Index_1]['OrganizationalStructure_Remark_Member'])
							$($('input[name = "OrganizationalStructure_Remark_Member"]')[set_Index_1]).val(get_Cooperative_OrganizationalStructure_Remark_Member[set_Index_1]['OrganizationalStructure_Remark_Member'])
						}

						else if (get_Element.attr('name') == 'OrganizationalStructure_Remark_Proof')
							get_Element.val(get_Cooperative_OrganizationalStructure_Remark_Member[set_Index_1]['OrganizationalStructure_Remark_Proof'])

						else if (get_Element.attr('name') == 'OrganizationalStructure_Remark_Address')
							get_Element.val(get_Cooperative_OrganizationalStructure_Remark_Member[set_Index_1]['OrganizationalStructure_Remark_Address'])
					})
				})
			})()
		...
		Initialize_Cooperative_EconomicSurvey()
		{
			$('form').on('submit', set_Event => { set_Event.preventDefault() })
			...
			this.Initialize_Table_Cooperative_Requested = (() =>
			{
				this.Table_Cooperative_Requested = $(this.Element['Table_Cooperative_Requested'].current)
				this.Table_Cooperative_Requested.DataTable().clear().destroy()

				this.Table_Cooperative_Requested.DataTable
				({
					'data'			:	this.state['Cooperative_Requested'],
					'columnDefs'	:
					[
						{
					...
					[
...

						{
							...
							{
								if (set_CellData['Cooperative_State'] == 'Pending' || set_CellData['Cooperative_State'] == 'Declined' || set_CellData['Cooperative_State'] == 'Cancelled')
									ReactDOM.render
									(
										<div className = 'd-flex justify-content-center flex-column'>
											<div className = 'p-1'>
												<button onClick = { () => this.Select_Cooperative_EconomicSurvey('Update Economic Survey', set_CellData['Cooperative_Identity'])} type = 'button' className = 'btn btn-info waves-effect DataTable_Button'>Update Economic Survey</button>
											</div>

											<div className = 'p-1'>
											{
												set_CellData['Cooperative_State'] == 'Pending'
												?
												<button onClick = { () => this.Manage_Cooperative_EconomicSurvey('Cancel Request', set_CellData['Cooperative_Identity'], set_Cell) } type = 'button' className = 'btn btn-danger waves-effect DataTable_Button'>Cancel Request</button>
												:
												set_CellData['Cooperative_State'] == 'Declined' || set_CellData['Cooperative_State'] == 'Cancelled'
												?
												<button onClick = { () => this.Manage_Cooperative_EconomicSurvey('Re-request', set_CellData['Cooperative_Identity'], set_Cell) } type = 'button' className = 'btn btn-warning waves-effect DataTable_Button'>Re-request</button>
												:
												<div></div>
											}
											</div>
								...
								else if (set_CellData['Cooperative_State'] == 'Confirmed')
								{
									let get_Position

									for (let get_Counter = 0; get_Counter <= this.state['User_Member_Information'].length; get_Counter ++)
									{
										if (set_CellData['Cooperative_Identity'] == this.state['User_Member_Information'][get_Counter]['Cooperative_Identity'])
										{
									...
									if (get_Position.includes('Dismissed'))
										ReactDOM.render
										(
											<div className = 'd-flex justify-content-center flex-column'>
												<div className = 'p-1'>
													<button onClick = { () => this.Manage_Cooperative_EconomicSurvey('Report', set_CellData['Cooperative_Identity'], set_Cell) } type = 'button' className = 'btn btn-info waves-effect DataTable_Button'>Report</button>
												</div>
											</div>
											,set_Cell
										)

									else if (get_Position.includes('Owner'))
									{
										ReactDOM.render
										(
											<div className = 'd-flex justify-content-center flex-column'>
												<div className = 'p-1'>
													<button onClick = { () => this.Select_Cooperative_EconomicSurvey('View Economic Survey', set_CellData['Cooperative_Identity']) } type = 'button' className = 'btn btn-info waves-effect DataTable_Button'>View Economic Survey</button>
												</div>
											</div>
											,set_Cell
										)
					...
					'initComplete'	:	() =>
					{
						...
						(
							...
							$(this.Element['Table_Cooperative_Requested'].current).parent().find('.dataTables_actions')[0]
						)
					},
				})
			})
		...
		get_Cooperative_EconomicSurvey_Interface()
		{
			return	<div>
										...
										<div className = 'tab-container'>
											<ul className = 'nav nav-tabs' role = 'tablist'>
												<li className = 'nav-item'>
													<a className = 'nav-link active' data-toggle = 'tab' href = '#Tab_Cooperative_Add' role = 'tab'>Cooperative Economic Survey</a>
												</li>

											{
												location.pathname != '/Cooperative/List'
												?
												<li className = 'nav-item'>
													<a className = 'nav-link' data-toggle = 'tab' href = '#Table_Cooperative_Requested' role = 'tab'>Requested Cooperative</a>
												</li>
												:
												<div></div>
											}
											...
											<div className = 'tab-content'>
												<div className = 'tab-pane fade active show' id = 'Tab_Cooperative_Add' role = 'tabpanel'>
													<div id = 'Carousel_Cooperative_EconomicSurvey' className = 'carousel slide' data-ride = 'carousel' data-interval = 'false'>
														<ol className = 'carousel-indicators'>
															<li data-target = '#Carousel_Cooperative_EconomicSurvey' data-slide-to = '0' className = 'active'></li>
															<li data-target = '#Carousel_Cooperative_EconomicSurvey' data-slide-to = '1'></li>
															<li data-target = '#Carousel_Cooperative_EconomicSurvey' data-slide-to = '2'></li>
															<li data-target = '#Carousel_Cooperative_EconomicSurvey' data-slide-to = '3'></li>
															<li data-target = '#Carousel_Cooperative_EconomicSurvey' data-slide-to = '4'></li>
															<li data-target = '#Carousel_Cooperative_EconomicSurvey' data-slide-to = '5'></li>
															<li data-target = '#Carousel_Cooperative_EconomicSurvey' data-slide-to = '6'></li>
															<li data-target = '#Carousel_Cooperative_EconomicSurvey' data-slide-to = '7'></li>
															<li data-target = '#Carousel_Cooperative_EconomicSurvey' data-slide-to = '8'></li>
															<li data-target = '#Carousel_Cooperative_EconomicSurvey' data-slide-to = '9'></li>
														</ol>
																...
																<form ref = { this.Element['Form_Cooperative_1'] }>
																	<div className = 'form-row'>
																		<div className = 'form-group col-md-12'>
																			<a className = 'page-link Carousel_Navigator float-left' href = '#Carousel_Cooperative_EconomicSurvey' role = 'button' data-slide = 'prev'>
																				<i className = 'zwicon-arrow-left'></i>
																			</a>

																			<a className = 'page-link Carousel_Navigator float-right' href = '#Carousel_Cooperative_EconomicSurvey' role = 'button' data-slide = 'next'>
																				<i className = 'zwicon-arrow-right'></i>
																			</a>
																		</div>

																		<div className = 'form-group col-md-12 pb-2 d-none'>
																			<input ref = { this.Element['Cooperative_Identity'] } name = 'Cooperative_Identity' className = 'form-control' type = 'text'/>
																			<label className = 'form-control-placeholder' htmlFor = 'Cooperative_Identity'>Identity</label>
																			<div className = 'valid-feedback'>Identity is valid</div>
																			<div className = 'invalid-feedback'>Please provide a Identity</div>
																		</div>
																	...
																	<div className = 'form-row'>
																		...
																		<div className = 'form-group col-md-4 pb-2'>
																			<select ref = { this.Element['Cooperative_Type'] } name = 'Cooperative_Type' className = 'form-control' data-placeholder = 'Select a Type' required>
																				...
																				{
																					this.state['Cooperative_Type'].map(set_Data =>
																					{
																						return	<option key = { set_Data['Type_Identity'] } value = { set_Data['Type_Identity'] }>{ set_Data['Type_Name'] }</option>
																					})
																				}
																				</optgroup>
																			</select>
																		...
																		<div className = 'form-group col-md-4 pb-2'>
																			<select ref = { this.Element['Cooperative_Category'] } name = 'Cooperative_Category' className = 'form-control' data-placeholder = 'Select a Category' required>
																				...
																				{
																					this.state['Cooperative_Category'].map(set_Data =>
																					{
																						return	<option key = { set_Data['Category_Identity'] } value = { set_Data['Category_Identity'] }>{ set_Data['Category_Name'] }</option>
																					})
																				}
																				</optgroup>
																			</select>
																		...
																		<div className = 'form-group col-md-4 pb-2'>
																			<select ref = { this.Element['Cooperative_Region'] } name = 'Cooperative_Region' className = 'form-control' data-placeholder = 'Select a Region' required>
																				...
																				{
																					this.state['Philippines_Region'].map(set_Data =>
																					{
																						return	<option key = { set_Data['Region_Identity'] } value = { set_Data['Region_Identity'] }>{ set_Data['Region_Name'] }</option>
																					})
																				}
																				</optgroup>
																			</select>
																		...
																		<div className = 'form-group col-md-4 pb-2'>
																			<select ref = { this.Element['Cooperative_Province'] } name = 'Cooperative_Province' className = 'form-control' data-placeholder = 'Select a Province' required>
																				...
																				{
																					this.state['Philippines_Region_Province'].map(set_Data =>
																					{
																						return	<option key = { set_Data['Province_Identity'] } value = { set_Data['Province_Identity'] }>{ set_Data['Province_Name'] }</option>
																					})
																				}
																				</optgroup>
																			</select>
																		...
																		<div className = 'form-group col-md-4 pb-2'>
																			<select ref = { this.Element['Cooperative_Locality'] } name = 'Cooperative_Locality' className = 'form-control' data-placeholder = 'Select a Locality' required>
																				...
																				{
																					this.state['Philippines_Region_Province_Locality'].map(set_Data =>
																					{
																						return	<option key = { set_Data['Locality_Identity'] } value = { set_Data['Locality_Identity'] }>{ set_Data['Locality_Name'] }</option>
																					})
																				}
																				</optgroup>
																			</select>
																		...
																		<div className = 'form-group col-md-4 pb-2'>
																			<select ref = { this.Element['Cooperative_Operation_Area'] } name = 'Cooperative_Operation_Area' className = 'form-control' data-placeholder = 'Select an Area of Operation' required>
																				...
																				{
																					this.state['Cooperative_OperationArea'].map(set_Data =>
																					{
																						return	<option key = { set_Data['OperationArea_Identity'] } value = { set_Data['OperationArea_Identity'] }>{ set_Data['OperationArea_Name'] }</option>
																					})
																				}
																				</optgroup>
																			</select>
																		...
																		<div className = 'form-group col-md-4 pb-2'>
																			<select ref = { this.Element['Cooperative_Common_Bond'] } name = 'Cooperative_Common_Bond' className = 'form-control' data-placeholder = 'Select a Common Bond' required>
																				...
																				{
																					this.state['Cooperative_CommonBond'].map(set_Data =>
																					{
																						return	<option key = { set_Data['CommonBond_Identity'] } value = { set_Data['CommonBond_Identity'] }>{ set_Data['CommonBond_Name'] }</option>
																					})
																				}
																				</optgroup>
																			</select>
																		...
																		<div className = 'form-group col-md-4 pb-2'>
																			<select ref = { this.Element['Cooperative_Organization_Registration'] } name = 'Cooperative_Organization_Registration' className = 'form-control' data-placeholder = 'Select a Government Organization' multiple required>
																				...
																				{
																					this.state['Philippines_Organization'].map(set_Data =>
																					{
																						return	<option key = { set_Data['Organization_Identity'] } value = { set_Data['Organization_Identity'] }>{ set_Data['Organization_Name'] }</option>
																					})
																				}
																				</optgroup>
																			</select>
																		...
																		<div className = 'form-group col-md-12 pb-2'>
																			<button className = 'btn btn-outline-theme btn--icon-text waves-effect' type = 'button' data-toggle = 'collapse' data-target = '#Collapse_Cooperative_Member_Projected_Growth'>
																	...
																	<div className = 'form-row'>
																		...
																		<div className = 'form-group col-md-4 pb-2'>
																			<select ref = { this.Element['EconomicAspect_Identical_Cooperative_Plan'] } name = 'EconomicAspect_Identical_Cooperative_Plan' className = 'form-control' data-placeholder = 'Select an identical Cooperative' multiple>
																				...
																				{
																					this.state['Cooperative'].map(set_Data =>
																					{
																						return	<option key = { set_Data['Cooperative_Identity'] } value = { set_Data['Cooperative_Identity'] }>{ set_Data['Cooperative_Name'] }</option>
																					})
																				}
																				</optgroup>
																			</select>
																	...
																	<div className = 'form-row'>
																		...
																		<div className = 'form-group col-md-4 pb-2'>
																			<select ref = { this.Element['FinancialAspect_Capitalization_Generate_Capital'] } name = 'FinancialAspect_Capitalization_Generate_Capital' className = 'form-control' data-placeholder = 'Select Capitalization' multiple required>
																				...
																				{
																					this.state['Cooperative_Capitalization'].map(set_Data =>
																					{
																						return	<option key = { set_Data['Capitalization_Identity'] } value = { set_Data['Capitalization_Identity'] }>{ set_Data['Capitalization_Description'] }</option>
																					})
																				}
																				</optgroup>
																			</select>
																		...
																		<div className = 'form-group col-md-12 pb-2'>
																			<button
																				onClick =
																				{
																					() =>
																					{
																						if ($(this.Element['Cooperative_Identity'].current).val() == '')
																							this.Manage_Cooperative_EconomicSurvey('Insert : Specific', this.get_Cooperative_EconomicSurvey_Data(), null)

																						else
																							this.Manage_Cooperative_EconomicSurvey('Update : Specific', this.get_Cooperative_EconomicSurvey_Data(), null)
																					}
																				}
...

											{
												location.pathname != '/Cooperative/List'
												?
												<div className = 'tab-pane fade' id = 'Table_Cooperative_Requested' role = 'tabpanel'>
													<h4 className = 'card-title'>Cooperative/s</h4>
													<h6 className = 'card-subtitle'>List of&nbsp;<span className = 'Highlight_Text'>Requested Cooperative/s</span></h6>

													<hr className = 'pb-4'/>

													<table ref = { this.Element['Table_Cooperative_Requested'] } className = 'table table-bordered table-hover' width = '100%'>
														<thead>
															<tr>
																<th>Name</th>
																<th>Type</th>
	...
	window.ECIMS_Cooperative_EconomicSurvey = ECIMS_Cooperative_EconomicSurvey
})