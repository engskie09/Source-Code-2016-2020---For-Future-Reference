...
class User:
	...
	def Select_User():
		...
		elif GLOBAL['REQUEST']['Select_Action'] == 'Select : Specific : { { Cooperative_Identity, Member_State } }':
			Query_Select = f"""
								SELECT * FROM User_Credential
								...
								INNER JOIN Cooperative_OrganizationalStructure_Member ON
								Cooperative_OrganizationalStructure_Member.Member_Identity = User_Credential.User_Identity

								WHERE

								User_Credential.User_Identity IN
								(
									Select Member_Identity FROM Cooperative_OrganizationalStructure_Member
	...
	def Select_User_Log():
		...
		elif GLOBAL['REQUEST']['Select_Action'] == 'Select : Specific : { User_Identity, Log_Type }':
			Query_Select = f"""
								SELECT * FROM User_Log
								WHERE
								User_Identity = '{ GLOBAL['REQUEST']['User_Identity'] }'
								AND
								Log_Type = '{ GLOBAL['REQUEST']['Log_Type'] }'
							"""

		elif GLOBAL['REQUEST']['Select_Action'] == 'Select : Specific : { User_Identity, Log_Type = { Etherium : Add, Etherium : Remove } }':
			Query_Select = f"""
								SELECT * FROM User_Log
								WHERE
								User_Identity = '{ GLOBAL['REQUEST']['User_Identity'] }'
								AND
								Log_Type IN ('{ GLOBAL['REQUEST']['Log_Type_1'] }', '{ GLOBAL['REQUEST']['Log_Type_2'] }')
							"""
	...
	def Insert_User_Log():
		...
		Query_Insert = f"""
							...
							(
								...
								Log_Information,
								Log_Inserted_By,
								Log_Inserted_DateTime
							)
							VALUES
							(
								'{ GLOBAL['REQUEST']['User_Identity'] }',
								'{ GLOBAL['REQUEST']['Log_Identity'] }',
								'{ GLOBAL['REQUEST']['Log_Action'] }',
								'{ GLOBAL['REQUEST']['Log_Type'] }',
								'{ GLOBAL['REQUEST']['Log_Information'] }',
								'{ GLOBAL['REQUEST']['Log_Inserted_By'] }',
								'{ GLOBAL['REQUEST']['Log_Inserted_DateTime'] }'
							)
						"""
		try:
			PyMySQL_Row_Count = GLOBAL['PyMySQL_Cursor'].execute(Query_Insert)
