...
/*-------------Engskie09_Supplier_Category-----------------*/
/*-------------Engskie09_Supplier_Category-----------------*/
/*-------------Engskie09_Supplier_Category-----------------*/
/*-------------Engskie09_Supplier_Category-----------------*/
/*-------------Engskie09_Supplier_Category-----------------*/

export function LECSteelMC_Supplier_Category_Controller($rootScope, $scope, $compile, Supplier_Information)
{
    var tbl_Supplier_Category_Information = angular.element('#tbl_Supplier_Category_Information')

    $scope.Module_Loading_Visible = true
    $scope.LECSteelMC_Supplier_Category_Visible = false

    $rootScope.set_Supplier_Category_Action = function(set_Action)
    {
        angular.element('#frm_Supplier_Category_Information').trigger('reset')
    ...
    $scope.Select_Supplier_Category_Information = function(set_Element, set_Action)
    {
        ...
        Supplier_Information.Select_Supplier_Category_Information(set_Action,
        ...
        function(set_Callback_Data)
        {
            if (set_Action['Action'] == 'Select : All')
            {
                ...
                tbl_Supplier_Category_Information.DataTable(
                {
                    ...
                    [                        
                        ...
                        {   orderable : false, 
                            ...
                            { 
                                return  "<button class = 'btn btn-outline-success mr-1 col-md' id = '" + data['Category_Identity'] + "' ng-click = '" + 'Select_Supplier_Category_Information($event, "Select : Specific");'  + "'>Edit</button>" +
                                        "<hr>" +
                                        "<button class = 'btn btn-outline-danger col-md' id = '" + data['Category_Identity'] + "' ng-click = '" + 'Manage_Supplier_Category_Information($event, "Delete_Supplier_Category_Information")' + "'>Delete</button>"
                            }
                        }                           
                    ],
                ...
                $scope.LECSteelMC_Supplier_Category_Visible = true                
            }

            else if (set_Action['Action'] == 'Select : Specific') 
            {                
                var get_JSON = JSON.parse(set_Callback_Data)
                $scope.set_Supplier_Category_Action('Update')                
    ...
    $scope.Manage_Supplier_Category_Information = function(set_Element, set_Action)
    {

        if (set_Action == 'Delete_Supplier_Category_Information')
        {
            ...
            }).then(function(set_Value)
            {
                if (set_Value != null)
                {
                    ...
                    Supplier_Information.Manage_Supplier_Category_Information(set_Action, JSON.stringify(get_Data),
                    function(set_Callback_Data)
                    {
                        if (set_Callback_Data)
                        {
                            ...
                            $scope.get_Dialog_Choice.Alert.Visible = true                      
                            tbl_Supplier_Category_Information.DataTable().destroy()

                            $scope.Select_Supplier_Category_Information(null, 'Select : All')
                        }
        ...
        {
            angular.element('#frm_Supplier_Category_Information').validate(
            {
                ...
                submitHandler: function()
                {
                    ...
                    if (get_Element.text() == 'Add')
                        set_Action = 'Insert_Supplier_Category_Information'

                    else if (get_Element.text() == 'Edit')
                        set_Action = 'Update_Supplier_Category_Information'
                    

                    setTimeout(function () 
                    {
                        var get_Data = new FormData(angular.element(document.querySelector('#frm_Supplier_Category_Information'))[0])

                        Supplier_Information.Manage_Supplier_Category_Information(set_Action, get_Data,                     
                        function(set_Callback_Data)
                        {
                            if (set_Callback_Data)
                            {
