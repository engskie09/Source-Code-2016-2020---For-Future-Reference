define('ECIMS_Cooperator_Account', () =>
{
	class ECIMS_Cooperator_Account extends ECIMS_Cooperator
	{
		...
		{
			if (!this.check_Component_Mutablity('Cooperator_Account'))
			{
		...
		get_Cooperator_Account_Interface()
		{
			return	<div>
									...
									<div className = 'card-body'>
										<div className = 'tab-container'>	
											<ul className = 'nav nav-tabs' role = 'tablist'>
												{
													this.check_Component_Mutablity('Cooperator_Account')
													?
														...
														<div className = 'container'>
															<div className = 'row'>
																...
																<div className = 'col-md-6 d-flex flex-column justify-content-center row'>
																	...
																	<div className = 'col-md Etherium_Border'>
																		<div className = 'col-md mt-2'>
																			...
																			{ System_Blockchain_User.CryptoCurrency_Transaction_Inteface(this) }
																			{ System_Blockchain_User.CryptoCurrency_Withdraw_Inteface(this) }
																			<div className = 'btn-toolbar d-flex justify-content-center' style = {{ 'fontSize' : '1.5em' }}>
																				<div className = 'row col-md-12'>

																					<button onClick = { () => $(this.Element['Dialog_CryptoCurrency_Add'].current).modal('show') } className = 'btn btn-theme btn--icon-text col-md waves-effect m-2' type = 'button'>
																				...
																				<div className = 'row col-md-12'>
																					<button onClick = { () => $(this.Element['Dialog_CryptoCurrency_Withdraw'].current).modal('show') } className = 'btn btn-theme btn--icon-text col-md waves-effect m-2' type = 'button'>
																					...
																					<button onClick =
																						{
																							() =>
																							{
																								...
																								this.Element['Dialog_CryptoCurrency_Transaction'].current.Show()
																								this.Element['Dialog_CryptoCurrency_Transaction'].current.Title = 'Loading Etherium Transaction'
																								this.Element['Dialog_CryptoCurrency_Transaction'].current.Size = 'Small'
																								this.Element['Dialog_CryptoCurrency_Transaction'].current.Closable = true
																								this.Element['Dialog_CryptoCurrency_Transaction'].current.Mode = 'Wait'
																								this.Element['Dialog_CryptoCurrency_Transaction'].current.Hide_Button()

																								let get_FormData = new FormData()
																								get_FormData.append('Action', 'Select_User_Log')
																								get_FormData.append('Select_Action', 'Select : Specific : { User_Identity, Log_Type = { Etherium : Add, Etherium : Remove } }')
																								get_FormData.append('User_Identity', Session_User_Identity())
																								get_FormData.append('Log_Type_1', 'Etherium : Add')
																								get_FormData.append('Log_Type_2', 'Etherium : Remove')

																								$(this.Element['Table_CryptoCurrency_Transaction'].current).DataTable().destroy()

																								ECIMS_Main_HTTP.Select(get_FormData, set_Callback_Data =>
																								{
																									$(this.Element['Table_CryptoCurrency_Transaction'].current).DataTable
																									({
																										'data'			:	set_Callback_Data,
																										'columnDefs'	:
																										[
																											{
																										...
																										[
...

																											{
																												...
																												'render'	:	set_Data =>
																												{
																													console.log(JSON.parse(set_Data['Log_Information']))
																													if (!Boolean(set_Data['Log_Information']))
																														return 'API Error'

																													if (set_Data['Log_Type'] == 'Etherium : Add')
																													{
																														let get_Data = JSON.parse(set_Data['Log_Information'])

																														let get_Amount = get_Data['purchase_units'][0]['amount']
																														let get_Amount_Currency_Code = get_Amount['currency_code']
																														let get_Amount_Value = get_Amount['value']

																														return  get_Amount_Currency_Code + ' ' + get_Amount_Value
																													}

																													else if (set_Data['Log_Type'] == 'Etherium : Remove')
																													{
																														let get_Data = JSON.parse(set_Data['Log_Information'])

																														let get_Amount = get_Data['batch_header']['amount']
																														let get_Amount_Currency_Code = get_Amount['currency']
																														let get_Amount_Value = get_Amount['value']
																										...
																										'initComplete'	:	() =>
																										{
																											...
																											(
																												...
																												$(this.Element['Table_CryptoCurrency_Transaction'].current).parent().find('.dataTables_actions')[0]
																											)

																											this.Element['Dialog_CryptoCurrency_Transaction'].current.Title = 'Etherium Transaction'
																											this.Element['Dialog_CryptoCurrency_Transaction'].current.Size = 'Large'
																											this.Element['Dialog_CryptoCurrency_Transaction'].current.Mode = 'Custom'
																											this.Element['Dialog_CryptoCurrency_Transaction'].current.Show_Button()
																										},
																									})
																								})
																							}
																						}
																						className = 'btn btn-theme btn--icon-text col-md waves-effect m-2'
																						type = 'button'>
